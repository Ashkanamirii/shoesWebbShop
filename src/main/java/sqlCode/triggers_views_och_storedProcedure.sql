...
create table no_stock(id int not null auto_increment primary key,
                      shoes_id int not null, end_date timestamp default current_timestamp on update current_timestamp);
...
create trigger update_no_stock
    ...
    for each row
    if (select quantity from shoes where id=new.id)<=0 then
...
create view colors AS select distinct color from shoes;
create view sizes AS select distinct size from shoes;
...
insert into order_line_item(FK_shoes_id,FK_order_id,quantity) values (10,9,10);
select * from shoes;
...
select id,FK_customer_id from orders where id=4;